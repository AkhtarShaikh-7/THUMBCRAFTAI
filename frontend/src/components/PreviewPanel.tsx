...
const PreviewPanel = ({thumbnail,isLoading, aspectRatio}: {thumbnail: IThumbnail | null,isLoading:boolean;aspectRatio:AspectRatio}) => {
  ...
  return (
    <div className='relative mx-auto w-full max-w-2xl'>
        <div className={`relative overflow-hidden rounded-xl bg-zinc-900 min-h-[350px] ${aspectClasses[aspectRatio]}`}>
            {isLoading && (
                ...
                !isLoading && thumbnail?.image_url && (
                    <div className="group relative h-full w-full">
                        ...
                        <div className='absolute inset-0 flex items-end justify-center bg-black/10 opacity-0 transition-opacity group-hover:opacity-100'>
                            <button onClick={onDownlaod} className='mb-6 flex items-center gap-2 rounded-md px-5 scroll-py-2.5 text-xs font-medium transition bg-white/30 ring-2 ring-white/40 backdrop-blur hover:scale-105 active:scale-95' type='button'>
